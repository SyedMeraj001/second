...
export class IoTDataIngestion {
  ...
  static async connectToIoTPlatform(platform) {
    try {
      const platforms = {
  ...
  static async ingestSensorData(sensorId) {
    try {
      const data = await this.fetchSensorData(sensorId);
      return this.processSensorData(data);
    } catch (error) {
      console.error('Sensor data ingestion failed:', error);
      return this.getMockSensorData('energy');
    }
  }

  static async fetchSensorData(sensorId) {
    const response = await fetch(`/api/iot/sensors/${encodeURIComponent(sensorId)}/data`, {
      method: 'GET',
  ...
  static processSensorData(rawData) {
    ...
    const processor = processors.energy || this.processEnergyData;
