...
export class IntegrationManager {
  ...
  static configure(options = {}) {
    ...
    import('./IntegrationConfig').then(({ IntegrationConfig }) => {
  ...
  static createFlexibleConnector(type, config = {}) {
    return import('./FlexibleConnector').then(({ FlexibleConnector }) => {
  ...
  static async initializeIntegrations(config) {
    ...
    if (config.erp) {
      for (const [system, credentials] of Object.entries(config.erp)) {
        try {
          ...
          results[`erp_${system}`] = { success: false, error: this.sanitizeError(error.message) };
        }
      }
    }
    ...
    if (config.iot) {
      try {
        ...
        results.iot = { success: false, error: this.sanitizeError(error.message) };
      }
    }
    ...
    if (config.hrms) {
      for (const [system, credentials] of Object.entries(config.hrms)) {
        try {
          ...
          results[`hrms_${system}`] = { success: false, error: this.sanitizeError(error.message) };
        }
      }
    }
  ...
  static async syncAllData(dateRange = {}) {
    ...
    for (const [key, connection] of this.connectedSystems.entries()) {
      try {
        ...
        syncResults[key] = { success: false, error: this.sanitizeError(error.message) };
      }
    }
  ...
  static async syncSingleSystem(systemKey, connection) {
    switch (connection.type) {
      case 'erp':
  ...
  static async testAllConnections() {
    ...
    for (const [key, connection] of this.connectedSystems.entries()) {
      try {
        ...
        results[key] = { success: false, error: this.sanitizeError(error.message) };
      }
    }
