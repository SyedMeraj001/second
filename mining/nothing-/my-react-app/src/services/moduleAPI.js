...
class ModuleAPI {
  ...
  static async request(endpoint, options = {}) {
    try {
      const sanitizedEndpoint = this.validateEndpoint(endpoint);
  ...
  static async calculateKPIs(companyId) {
    try {
      // Validate and sanitize companyId to prevent SSRF
      if (!companyId || typeof companyId !== 'string') {
        throw new Error('Invalid companyId');
      }
      const sanitizedCompanyId = encodeURIComponent(companyId.replace(/[^a-zA-Z0-9_-]/g, ''));
      
      const response = await fetch(`http://localhost:5000/api/kpi/${sanitizedCompanyId}`);
