...
import { FaEnvelope, FaLock, FaEyeSlash, FaLeaf, FaUser } from 'react-icons/fa';
import logo from './companyLogo.jpg';
import { initializePreconfiguredUsers } from './utils/rbac';
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isSignup) {
      ...
      try {
        ...
        setMessage('Error exploring to backend');
      }
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        if (data.requires2FA) {
          // Trigger 2FA Modal
          setPendingUser({ id: data.userId, email });
          setShow2FA(true);
        } else {
  ...
  const handle2FAVerify = () => {
    // 2FA Logic to be implemented with backend verification
    setShow2FA(false);
    // completeLogin(pendingUser, pendingToken); 
  };
        ...
        <div className="form-box Login absolute top-0 left-0 w-1/2 h-full flex justify-center flex-col px-10">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className={`input-box animation relative w-full h-12 mt-6 transition-all duration-700 ${!isSignup ? 'transform translate-x-0 opacity-100' : 'transform -translate-x-full opacity-0'}`} style={{
              ...
              />
              <label htmlFor="login-username" className={`absolute top-1/2 left-0 transform -translate-y-1/2 text-base transition-all duration-500 ${isDark ? 'text-white' : 'text-gray-900'} ${email ? 'top-[-5px] text-[#3a7a44]' : ''}`}>{'Username'}</label>
            ...
            <div className={`input-box animation relative w-full h-12 mt-6 transition-all duration-700 ${!isSignup ? 'transform translate-x-0 opacity-100' : 'transform -translate-x-full opacity-0'}`} style={{
              ...
              <input
                ...
                className={`w-full h-full bg-transparent border-none outline-none text-base font-semibold border-b-2 pr-6 transition-all duration-500 ${isDark ? 'text-white border-white focus:border-[#3a7a44]' : 'text-gray-900 border-gray-900 focus:border-[#3a7a44]'}`}
              />
              <label htmlFor="login-password" className={`absolute top-1/2 left-0 transform -translate-y-1/2 text-base transition-all duration-500 ${isDark ? 'text-white' : 'text-gray-900'} ${password ? 'top-[-5px] text-[#3a7a44]' : ''}`}>{'Password'}</label>
              <button
                type="button"
          ...
          <form onSubmit={handleSubmit}>
            <div className={`input-box animation relative w-full h-12 mt-6 transition-all duration-700 ${isSignup ? 'transform translate-x-0 opacity-100 blur-0' : 'transform translate-x-full opacity-0 blur-sm'}`} style={{
              ...
              <input
                ...
                className={`w-full h-full bg-transparent border-none outline-none text-base font-semibold border-b-2 pr-6 transition-all duration-500 ${isDark ? 'text-white border-white focus:border-[#3a7a44]' : 'text-gray-900 border-gray-900 focus:border-[#3a7a44]'}`}
              />
              <label htmlFor="signup-fullname" className={`absolute top-1/2 left-0 transform -translate-y-1/2 text-base transition-all duration-500 ${isDark ? 'text-white' : 'text-gray-900'} ${fullName ? 'top-[-5px] text-[#3a7a44]' : ''}`}>{'Username'}</label>
            ...
            <div className={`input-box animation relative w-full h-12 mt-6 transition-all duration-700 ${isSignup ? 'transform translate-x-0 opacity-100 blur-0' : 'transform translate-x-full opacity-0 blur-sm'}`} style={{
              ...
              <input
                ...
                className={`w-full h-full bg-transparent border-none outline-none text-base font-semibold border-b-2 pr-6 transition-all duration-500 ${isDark ? 'text-white border-white focus:border-[#3a7a44]' : 'text-gray-900 border-gray-900 focus:border-[#3a7a44]'}`}
              />
              <label htmlFor="signup-email" className={`absolute top-1/2 left-0 transform -translate-y-1/2 text-base transition-all duration-500 ${isDark ? 'text-white' : 'text-gray-900'} ${email ? 'top-[-5px] text-[#3a7a44]' : ''}`}>{'Email'}</label>
            ...
            <div className={`input-box animation relative w-full h-12 mt-6 transition-all duration-700 ${isSignup ? 'transform translate-x-0 opacity-100 blur-0' : 'transform translate-x-full opacity-0 blur-sm'}`} style={{
              ...
              <input
                ...
                className={`w-full h-full bg-transparent border-none outline-none text-base font-semibold border-b-2 pr-6 transition-all duration-500 ${isDark ? 'text-white border-white focus:border-[#3a7a44]' : 'text-gray-900 border-gray-900 focus:border-[#3a7a44]'}`}
              />
              <label htmlFor="signup-password" className={`absolute top-1/2 left-0 transform -translate-y-1/2 text-base transition-all duration-500 ${isDark ? 'text-white' : 'text-gray-900'} ${password ? 'top-[-5px] text-[#3a7a44]' : ''}`}>{'Password'}</label>
