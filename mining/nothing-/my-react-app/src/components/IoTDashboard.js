...
const IoTDashboard = ({ onClose }) => {
  ...
  const fetchDeviceStatus = async () => {
    try {
      const response = await fetch('/api/iot/devices/status');
  ...
  const fetchRealTimeMetrics = async () => {
    try {
      const response = await fetch('/api/iot/metrics/realtime');
      const data = await response.json();
      if (data.success) {
  ...
  const fetchDeviceAnalytics = async () => {
    try {
      await fetch(`/api/iot/devices/analytics?timeRange=${encodeURIComponent(timeRange)}`);
    } catch (error) {
  ...
  const fetchESGImpact = async () => {
    try {
      const response = await fetch('/api/iot/metrics/esg-impact');
      const data = await response.json();
      if (data.success) {
  ...
  const ingestSensorData = async (data) => {
    try {
      const response = await fetch('/api/iot/data/ingest', {
        method: 'POST',
  ...
  const viewDeviceDetails = async (deviceId) => {
    try {
      const response = await fetch(`/api/iot/data/history/${encodeURIComponent(deviceId)}?hours=24`);
      const data = await response.json();
      if (data.success) {
  ...
  const deleteDevice = async (deviceId) => {
    if (window.confirm(`Are you sure you want to delete device ${deviceId}? This will also delete all associated sensor data.`)) {
      try {
        const response = await fetch(`/api/iot/devices/${encodeURIComponent(deviceId)}`, {
          method: 'DELETE'
        });
        const result = await response.json();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`lg:col-span-2 rounded-lg p-6 border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <h3 className={`text-xl font-semibold ${theme.text.primary} mb-4`}>{'📡 Connected Devices'} ({filteredDevices.length})</h3>
