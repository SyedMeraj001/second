...
const TwoFactorAuth = ({ onVerify, onCancel, email }) => {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/auth/2fa/validate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: email.id || email, token: fullCode }) // email prop might be userId object from Login.jsx
      });

      const data = await response.json();

      if (response.ok) {
        onVerify(data); // Pass full user data back to Login
      } else {
