...
const TwoFactorSetup = ({ onComplete, onCancel, userEmail }) => {
  ...
  const handleMethodSelect = async (selectedMethod) => {
    ...
    if (selectedMethod === 'app') {
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/auth/2fa/setup`, {
          method: 'POST',
  ...
  const handleVerify = async () => {
    if (verificationCode.length === 6) {
      try {
        const token = localStorage.getItem('authToken');
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/auth/2fa/verify`, {
          method: 'POST',
          headers: {
          ...
          <>
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 text-sm">
                We've sent a code to {method === 'email' ? userEmail : '+1 (***) ***-1234'}
              </p>
            </div>
