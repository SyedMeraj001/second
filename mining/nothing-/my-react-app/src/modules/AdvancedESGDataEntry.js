...
const AdvancedESGDataEntry = ({ onClose }) => {
  ...
  const handleSave = () => {
    // Ensure secure connection for sensitive ESG data
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
      console.warn('Insecure connection detected. ESG data should be transmitted over HTTPS.');
    }
    ...
    const existing = (() => {
      try {
        const stored = localStorage.getItem('advanced_esg_data') || btoa('[]');
        return JSON.parse(atob(stored));
      } catch {
        return [];
      }
    })();
    // Validate existing data structure for security
    if (!Array.isArray(existing)) {
      console.warn('Invalid data structure detected, resetting storage');
