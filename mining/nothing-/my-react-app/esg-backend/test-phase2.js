...
const BASE_URL = process.env.API_BASE_URL || 'http://localhost:3003/api';
...
async function testPhase2() {
  ...
  try {
    ...
    const erpConfig = await axios.post(`${BASE_URL}/integrations/erp/configure`, {
      type: 'SAP',
      baseURL: process.env.SAP_BASE_URL || 'https://api.sap-demo.com',
      apiKey: process.env.SAP_API_KEY || 'demo-key'
    });
    ...
    const hrConfig = await axios.post(`${BASE_URL}/integrations/hr/configure`, {
      type: 'Workday',
      baseURL: process.env.WORKDAY_BASE_URL || 'https://api.workday-demo.com',
      apiKey: process.env.WORKDAY_API_KEY || 'demo-key'
    });
    ...
    try {
      ...
      console.log('⚠️ Compliance validation failed:', error.response?.data?.error || error.message);
    }
    ...
    try {
      ...
      console.log('⚠️ Compliance requirements failed:', error.response?.data?.error || error.message);
    }
    ...
    try {
      ...
      console.log('⚠️ XBRL mappings failed:', error.response?.data?.error || error.message);
    }
