...
router.post('/targets/track', (req, res) => {
  try {
    ...
    const { currentValue, targetValue, targetYear } = req.body;
    ...
    res.json({
      ...
      recommendations: generateTargetRecommendations(annualReduction)
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
function generateRecommendations(comparison) {
  return Object.entries(comparison)
    .filter(([, data]) => data.performance === 'below_average')
    .map(([metricName]) => `Improve ${metricName} performance`);
}
...
function generateTargetRecommendations(reduction) {
