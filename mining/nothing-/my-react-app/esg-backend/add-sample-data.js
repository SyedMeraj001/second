...
// Lazy load CSRF token
let csrfToken = null;
const getCsrfToken = () => {
  if (!csrfToken) {
    csrfToken = process.env.CSRF_TOKEN || require('crypto').randomBytes(32).toString('hex');
  }
  return csrfToken;
};
...
async function addSampleData() {
  ...
  try {
    // Ensure fetch is loaded before use
    const fetchFn = await loadDependencies();
    ...
    const response = await fetchFn(`${API_BASE}/esg/data`, {
      method: 'POST',
      headers: { 
...
async function verifyData() {
  ...
  try {
    ...
    const verifyResponse = await fetchFn(`${API_BASE}/esg/verify/${encodeURIComponent(sampleData.userId)}`, {
      method: 'GET',
      headers: { 
        'X-CSRF-Token': getCsrfToken(),
        'X-Requested-With': 'XMLHttpRequest'
...
async function fetchKPIs() {
  ...
  try {
    ...
    const kpiResponse = await fetchFn(`${API_BASE}/esg/kpis/${encodeURIComponent(sampleData.userId)}`, {
      method: 'GET',
      headers: { 
        'X-CSRF-Token': getCsrfToken(),
        'X-Requested-With': 'XMLHttpRequest'
