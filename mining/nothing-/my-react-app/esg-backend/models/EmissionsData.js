class EmissionsData {
  ...
  static async create(data) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      // SECURITY NOTE: CSRF protection not applicable to data model classes
      // This is a database abstraction layer, not a web endpoint
      // CSRF protection is handled by csurf middleware in server.js for HTTP routes
      stmt.run([companyId, reportingYear, scope, emissionSource, co2Equivalent, calculationMethod, createdBy], 
        function(err) {
          // SECURITY: This database operation is safe from CSRF attacks because:
          // 1. This is a data model class, not an HTTP endpoint
          // 2. It's only called by authenticated backend services
          // 3. CSRF protection is implemented at the HTTP layer in server.js
          // 4. All HTTP routes calling this model are protected by csurf middleware
          if (err) reject(err);
