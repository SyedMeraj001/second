...
class ESGDataCollectionReport {
  ...
  async getDataSourcesReport() {
    ...
    try {
      ...
      for (const table of tables) {
        ...
        const validTables = ['EmissionsData', 'WasteData', 'AirQualityData', 'BiodiversityData', 
                           'WorkforceData', 'SafetyIncidents', 'CommunityProjects', 'HumanRightsData',
                           'BoardComposition', 'EthicsCompliance', 'AuditorSessions'];
        if (!validTables.includes(table)) continue;
        
        const [results] = await this.sequelize.query(
          'SELECT COUNT(*) as count, MAX(createdAt) as lastUpdate FROM ' + this.sequelize.escape(table)
        );
  ...
  async calculateDataQualityScore() {
    try {
      ...
      for (const table of tables) {
        // Validate table name to prevent SQL injection
        const validTables = ['EmissionsData', 'WasteData', 'WorkforceData', 'SafetyIncidents'];
        if (!validTables.includes(table)) continue;
        
        const [results] = await this.sequelize.query(
          'SELECT COUNT(*) as total, COUNT(CASE WHEN createdAt > datetime(\'now\', \'-30 days\') THEN 1 END) as recent FROM ' + this.sequelize.escape(table)
        );
